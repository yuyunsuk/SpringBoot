...
public class CustomExceptionHandler {
    ...
    protected ResponseEntity<BaseResponse<Map<String, String>>> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        Map<String, String> errors = new HashMap<String, String>(); // Key, Value[Error 기준]

        ex.getBindingResult().getAllErrors().forEach(error -> {

            String fieldName = ((FieldError) error).getField(); // Key => Field 명
            String errorMessage = error.getDefaultMessage();    // Value => message 를 getDefaultMessage 에 넣어줌

            errors.put(fieldName, errorMessage != null ? errorMessage
                    : "No Exception Message"); // 3항 연산자
        });
