...
public class MyAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    @Override
    public void commence(HttpServletRequest request, HttpServletResponse response, // Servlet 아주 사이즈가 작은 app
                         AuthenticationException authException)
            throws IOException, ServletException, JsonProcessingException {
        // 인증되지 않은 사용자가 보호된 리소스에 접근했을 때 호출됨

        // 응답으로 전송할 JSON 데이터 생성
        Map<String, Object> responseData = new HashMap<>(); // 응답을 만듦
        ...
        // 응답 헤더 설정
        response.setContentType("application/json"); // 응답 데이터 json 형태임
        response.setCharacterEncoding("UTF-8");
        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);

        // 응답 내용 출력
        PrintWriter out = response.getWriter();
        out.print(jsonResponse); // out 에 데이터를 씀
        out.flush(); // 내보냄
    }
}
